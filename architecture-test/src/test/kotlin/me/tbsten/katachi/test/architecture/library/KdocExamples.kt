 ...
 *
 * Shared so that eight roles print one sentence rather than eight spellings of it, in the same
 * way `laterLayersOf` is shared. The `konsist { }` call itself stays at the role — see
 * `LayerImports.kt` for why a helper must not wrap it.
 */
