...
**走査の前段のうち、モジュール探索もこの規則の対象**。ディレクトリ 1 つが読めなくても隣の
サブツリーは独立してモジュールを答えられるので、子 1 つ単位で握り、`UncheckedDirectory`
（`reason = ModulesNotDiscovered`）として報告に載せる。一方**ルート探索は握らない** —
ルートが決まらなければ検査するものが 1 つも残らず、「隣」が存在しないため。
